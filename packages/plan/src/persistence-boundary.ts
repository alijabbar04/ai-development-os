...
export interface PlanPersistenceAuthority {
  /** Trusted application ownership: atomically take a private identity capability. */
  take(authorization: PlanCommitAuthorization, request: PlanCommitRequest): IssuedPlanCommitFacts | null;
  /** Application verifies the consumed scope pair and journal in this transaction. */
  readConsumedScopeApproval?(tx: TransactionContext, request: PlanCommitRequest): Promise<ApprovalRequest | null>;
}
...
function startEvidenceDeadline(options: PlanPersistenceOptions): EvidenceDeadline {
...
function factsFor(
  factsValue: IssuedPlanCommitFacts | null,
  request: PlanCommitRequest,
): IssuedPlanCommitFacts | null {
  if (factsValue === null) return null;
  const facts = freezeFacts(factsValue);
  if (facts === undefined
...
function eventPlanAfterC6Transition(
  ...
  sealedByApprovalId: string | null = plan.sealedByApprovalId,
): ProjectPlan {
  try {
    const state = transition(PLAN_STATE_MACHINE, plan.state, event);
    return parseProjectPlan({ ...plan, state, updatedAt, sealedAt, sealedByApprovalId });
  }
...
function expectedFromCurrent(current: ProjectPlan, event: PlanHeadEventPayload, occurredAt: string): ProjectPlan {
  switch (event.kind) {
    case "plan.proposed":
      return eventPlanAfterC6Transition(current, event.operation.kind === "approve-scope" ? "scope-approval-consumed" : "validation-passed", occurredAt, null);
    ...
    case "plan.sealed":
      return eventPlanAfterC6Transition(current, "seal", occurredAt, occurredAt, event.seal.sealedByApprovalId);
...
function predecessorStamp(plan: ProjectPlan): PlanPredecessorEvidence {
  ...
  return Object.freeze({
    ...
    sealedByApprovalId: plan.sealedByApprovalId,
...
async function commit(
  ...
  authority: PlanPersistenceAuthority,
  ...
  // A deadline that expires before dispatch consumes no capability. Once the
  // transaction is dispatched the trusted owner's take remains strictly one-use.
  try { facts = factsFor(authority.take(authorization, request), request); }
  catch { facts = null; }
  ...
  try {
    const finalVersion = await adapter.transact(async (tx) => {
      ...
      const scopeApproval = await authority.readConsumedScopeApproval?.(tx, request) ?? null;
      if (request.steps.some((step) => step.event.operation.kind === "approve-scope") && scopeApproval === null) {
        throw refusal("PLAN_AUTHORITY_VIOLATION", "plan.seal.approval-binding");
      }
      for (const step of request.steps) {
        const event = step.event;
        if (step.expectedState !== (head?.plan.state ?? null)) throw new ExpectedConflict(aggregateVersion);
        if (head !== null && ["promote", "approve-scope", "require-scope-approval", "seal"].includes(event.operation.kind)
        ...
        if (event.kind === "plan.sealed") {
          ...
          if (event.seal === null
            ...
            || event.seal.sealedByApprovalId !== (scopeApproval?.approvalRequestId ?? null)
...
export function createPlanPersistenceBoundary(
  adapter: PersistenceAdapter,
  authority: PlanPersistenceAuthority,
  digest: PlanDigestPort = planSha256,
  options: PlanPersistenceOptions = Object.freeze({}),
): PlanStore {
  return Object.freeze({
    ...
    commit: (request: PlanCommitRequest, authorization: PlanCommitAuthorization): Promise<PlanCommitOutcome> => commit(adapter, request, authorization, digest, options, authority),
