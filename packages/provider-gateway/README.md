...
Registrations reject duplicate instances, ambiguous catalog bindings, model/descriptor mismatches, profile mismatches, cross-instance `SecretRef` reuse, and references bound to another provider instance. Configuration fingerprints exclude runtime health and quota observations, which are retrieved through separate typed ports. User preference and free-tier eligibility are independent fields. `verified-free-only` registration fails closed when catalog verification expires.
