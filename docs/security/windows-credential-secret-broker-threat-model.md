...
| Callback error collapse | The broker deliberately maps thrown consumer errors to fixed `CONSUMER_FAILURE` after material disposal. The testing-only Anthropic canary therefore returns an exact finite provider success/failure outcome through the callback and reconstructs its code plus conservative effect phase only after the broker settles. A broker/audit/result failure becomes `CALLBACK_RESULT_FAILURE`, never a fabricated transport classification. After the effect deadline, a fixed drain ceiling waits for cooperative settlement; expiry makes no disposal/no-effect claim about a nonconforming injected boundary. Raw callback, native, HTTP, response, and audit details remain excluded. |
