...
| R16: eighth exact-head run `32809415978` failed only the final Windows credential-host Electron smoke at `validation-in-flight-ui-lock` | The run is retained as failed product evidence and is not rerun. Its other six jobs passed, and the Windows job had already passed the 27-minute full check, exact manifest guard, native build/shape, and app-vault smoke before the default-mode report failed at `renderer-crash-complete`. R14 made cancel/close accounting synchronous and exact, but the renderer's defensive recovery from Chromium's later non-cancelable repeated-Escape close only called `showModal()` and relied on browser-default focus restoration. Retained disposable evidence shows the third Escape can be unprevented, close the dialog, and trigger same-dialog recovery; local Chromium happened to preserve the busy-heading focus, while hosted Windows did not reliably do so. The shared in-flight recovery now reopens the same connected dialog, waits for the next animation frame while rechecking that the same dialog remains connected and open, and then explicitly focuses its existing `[data-busy-focus="true"]` heading. Validation, removal, and credential-write dialogs all use the same recovery rule. The complete smoke predicate still requires the same dialog, exact event accounting, open-loss recovery, locked controls, visible wording, and busy-heading focus after every Escape. After the unchanged three-Escape assertions, the synthetic smoke now deterministically closes that same dialog once, explicitly blurs the busy heading inside every actual close event, and requires the production recovery to restore it while every lock remains intact. Focused browser-source and static regressions pin show-before-frame-before-recheck-before-focus ordering, all three production call sites, deliberate focus displacement, and the forced-close recovery. This changes no provider, credential, authorization, marker, receipt, policy, dispatch, retry, or production-activation behavior. Fresh repeated synthetic Electron evidence, focused/full gates, independent review, manifest rebinding, and new exact-head CI are required. |
| R17: repeated exact-R16 pre-publication smoke failed once at `warning-tone-and-postcommit-validation-authority` | The failed seventh cycle is retained as local product evidence; the other six cycles before it and ten immediately following diagnostic cycles passed all 77 assertions. Source-order inspection identified a real harness race: the renderer intentionally emits an intermediate committed `Saved securely` render while authoritative refresh is still in flight, so the old wait for only the title and existence of the validation action could complete while that action was still temporarily locked. The harness now derives one reusable warning-state snapshot, polls the complete unchanged assertion predicate through the final authoritative render, and then records that same snapshot. A focused static regression pins complete warning copy, validation authority, three locked storage actions, and wait-before-sample ordering. This changes only synthetic observation timing; no production renderer, provider, credential, authorization, marker, receipt, policy, dispatch, retry, or production-activation behavior changes. Fresh repeated smoke, full gates, independent review, manifest rebinding, and exact-head CI are required. |
