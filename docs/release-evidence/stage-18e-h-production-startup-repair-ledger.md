...
- Publication state at this source/evidence record: final independent review has passed. The superseded canonical manifest is intentionally absent from this source/evidence candidate; after commit it will be regenerated from committed Git blobs and re-added in a manifest-only child. Non-force push, exact-head hosted CI, ref equality, and clean-worktree proof then remain. Immutable publication identities will be recorded outside this commit in the required handover.
