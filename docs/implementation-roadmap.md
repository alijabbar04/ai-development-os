...
- Operator-present Stage 17W attempt (2026-08-13; failed closed, not a
  lifecycle proof): corrected pure preflight matched the exact implementation,
  SDK, controller/installer vectors, 193-file plan, source envelope, and four
  proof-artifact hashes. The exact reviewed installer and required remover then
  both returned exit code `2` before the ordinary controller ran. Independent
  post-checks found one empty token-derived protected leaf and zero process,
  profile, package, task-directory, marker, or registry residue. The leaf is
  preserved for manual review; no alternate cleanup, retry, lifecycle, egress,
