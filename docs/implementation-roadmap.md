...
- Child environments constructed from an empty baseline with a finite platform allowlist, broker-owned temporary/profile/config locations, deterministic locale, and an explicit PATH. A supplied session profile appears only as `HOME` on POSIX or `USERPROFILE` on Windows; the ambient home, opposite platform name, Windows legacy components, SSH, askpass, credential-helper, cloud, registry, CI, language-startup, Git-redirection, and proxy values are not inherited. Requests cannot bind the profile/config redirectors. Secrets resolve late, are scoped to one invocation, are literally redacted from captured output across chunk boundaries, and never reach results, audit records, or errors.
...
Anthropic authentication boundary: no Claude credential file, OAuth token, session cookie, API key, or credential-helper output is ever read, parsed, copied, exported, logged, serialized, or returned. There is no subscription-token field. Secret-backed environment bindings carry only a reference fingerprint and resolve through the Stage 8 resolver after policy approval, immediately before process creation. A personal installed-CLI login is refused without an explicit development-canary opt-in and is never presented as distributable. The boundary is mechanical rather than advisory: the process broker builds the child environment from an empty baseline, then supplies a fresh broker-owned session home as `HOME` on POSIX or `USERPROFILE` on Windows rather than inheriting the invoking user's home. The opposite home name, Windows legacy home components, and this adapter's XDG redirectors remain absent, and request bindings cannot override them. This prevents default installed-login discovery while preserving explicitly authorized API-key authentication. It redirects discovery but does not make user-readable filesystem paths unreachable under the unsafe development backend.

L-01 correction: the original Claude security test unconditionally rejected `HOME`, so it failed on POSIX and passed on Windows without checking `USERPROFILE`. Contract reconstruction also found that Node's Windows spawn path copied ambient `HOMEDRIVE` and `HOMEPATH` into a child even though the broker's constructed object omitted them. The bounded repair suppresses those parent values for the synchronous spawn call, restores the parent immediately, and adds simulated POSIX/Windows/null-home/override tests plus a real fake-CLI assertion over the selected platform home, session-root containment, freshness, cleanup, and absent opposite/legacy/XDG names. Exact corrective commit `6bd96c4005f24c575e5e4d849a9d5299e2901a08` passed the Ubuntu and Windows checks and the Windows coverage job in GitHub Actions run `31226295963`; L-01 is closed. That run's separate dependency-audit job reported newly published `GHSA-2v37-7h3g-55p8` against the unchanged transitive `nanoid@3.3.16` entry, which remains visible for a separately scoped dependency update.
