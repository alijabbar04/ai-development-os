...
L-01 correction checkpoint: the original Claude security test unconditionally rejected `HOME`, so it failed on POSIX and passed on Windows without checking `USERPROFILE`. Contract reconstruction also found that Node's Windows spawn path copied ambient `HOMEDRIVE` and `HOMEPATH` into a child even though the broker's constructed object omitted them. The bounded repair suppresses those parent values for the synchronous spawn call, restores the parent immediately, and adds simulated POSIX/Windows/null-home/override tests plus a real fake-CLI assertion over the selected platform home, session-root containment, freshness, cleanup, and absent opposite/legacy/XDG names. The implementation is locally verified but L-01 remains open until the exact repair commit passes both GitHub-hosted operating systems.
