...
- Package dry-runs contain no source, unit-test, live-canary, cache, or build-state files: catalog 26 files / 21,316 packed bytes, gateway 22 / 10,085, OpenAI-compatible 46 / 28,985, and Gemini 34 / 23,687. Build-only TypeScript configs prevent clean builds from emitting unit/live tests, while manifest exclusions also protect against stale local `dist` outputs.
