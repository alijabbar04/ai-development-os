...
Stages 0 through 16 are complete. Stage 17 remains gated. The Windows profile lifecycle and one synthetic zero-capability AppContainer plus creation-time private-Job composition now pass with zero measured residue. The smallest next Windows action is a separately authorized structured fixture proving allowed staged access, denied ungranted same-user filesystem access, a bounded no-capability network-denial slice, and rapid child-creation behavior. A production helper, packaging, crash cleanup, quotas, and the complete 40-vector actual-native corpus remain required. Linux, macOS, and controlled service egress still require their own implementations and actual-platform evidence. Platform-specific primitives, privileges, and installation must be measured rather than assumed.
