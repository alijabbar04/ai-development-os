...
export const STAGE_18E_H_MANIFEST_PATH = "docs/release-evidence/stage-18e-h-subject-manifest.json";
export const STAGE_18E_H_MANIFEST_ALGORITHM = "ai-dev-os.stage-18e-h.git-blob-subject.v1";
...
export function assertSubjectPath(path) {
  if (typeof path !== "string" || path.length === 0 || path.startsWith("/") || /^[A-Za-z]:\//u.test(path) || path.includes("\\") || /[\u0000\r\n\t]/u.test(path) || path.split("/").some((part) => part.length === 0 || part === "." || part === "..")) fail("SUBJECT_PATH_REFUSED", String(path));
  return path;
}

export function compareSubjectPaths(left, right) {
...
export function parseNameStatusZ(bytes) {
  ...
  for (let index = 0; index < fields.length; index += 2) {
    ...
    const path = assertSubjectPath(fields[index + 1]);
  ...
  entries.sort((left, right) => compareSubjectPaths(left.path, right.path));
  return Object.freeze(entries);
}

export function aggregateSubjectFiles(files) {
  const digest = createHash("sha256");
  let previousPath = null;
  for (const file of files) {
    assertSubjectPath(file.path);
    if (!HASH.test(file.sha256) || !OID.test(file.blobOid) || !Number.isSafeInteger(file.byteCount) || file.byteCount < 0) fail("SUBJECT_FILE_RECORD_REFUSED", file.path);
    if (previousPath !== null && compareSubjectPaths(previousPath, file.path) >= 0) fail("SUBJECT_FILE_ORDER_REFUSED", file.path);
...
export function firstManifestDifference(actual, expected, path = "manifest") {
  ...
  if (Array.isArray(actual) || Array.isArray(expected)) {
    ...
    for (let index = 0; index < expected.length; index += 1) {
      const difference = firstManifestDifference(actual[index], expected[index], `${path}[${index}]`);
  ...
  if (typeof actual === "object" && actual !== null && typeof expected === "object" && expected !== null) {
    ...
    for (const key of expectedKeys) {
      const difference = firstManifestDifference(actual[key], expected[key], `${path}.${key}`);
      if (difference !== null) return difference;
    }
    return null;
  }
...
export function assertManifestEquivalent(actual, expected) {
  const difference = firstManifestDifference(actual, expected);
  if (difference !== null) fail("SUBJECT_MANIFEST_DRIFT", difference);
}

export function runGit(repositoryRoot, arguments_, options = {}) {
  const result = spawnSync("git", arguments_, { cwd: repositoryRoot, encoding: options.binary === true ? "buffer" : "utf8", maxBuffer: 64 * 1024 * 1024, windowsHide: true });
  if (result.error !== undefined || result.status !== 0) fail("SUBJECT_GIT_FAILED", arguments_.join(" "));
...
export function readCommittedSubjectManifest(repositoryRoot, revision) {
  ...
  const bytes = runGit(repositoryRoot, ["cat-file", "blob", `${commit}:${STAGE_18E_H_MANIFEST_PATH}`], { binary: true });
...
export function collectSubjectManifest(repositoryRoot, baseRevision, sourceRevision) {
  ...
  const entries = parseNameStatusZ(runGit(repositoryRoot, ["diff", "--name-status", "-z", "--no-renames", baseCommit, sourceCommit], { binary: true }));
  if (entries.some((entry) => entry.path === STAGE_18E_H_MANIFEST_PATH)) fail("SUBJECT_MANIFEST_SELF_REFERENCE");
  const files = [];
  const deletedPaths = [];
  let totalBytes = 0;
  for (const entry of entries) {
    if (entry.status === "D") { deletedPaths.push(entry.path); continue; }
    ...
    const byteCount = blob.byteLength;
    totalBytes += byteCount;
    files.push(Object.freeze({ path: entry.path, changeType: entry.status, byteCount, blobOid, sha256: createHash("sha256").update(blob).digest("hex") }));
  }
  const manifest = {
    schemaVersion: 1,
    algorithm: STAGE_18E_H_MANIFEST_ALGORITHM,
    baseCommit,
    sourceCommit,
    sourceTree,
    manifestPath: STAGE_18E_H_MANIFEST_PATH,
    rules: {
    ...
    excludedPaths: [
      {
        path: STAGE_18E_H_MANIFEST_PATH,
        reason: "not present in sourceCommit; added only by its manifest-only child commit to avoid self-reference",
      },
    ],
    aggregateSha256: aggregateSubjectFiles(files),
    files,
    generator: {
      identity: "scripts/generate-stage-18e-h-subject-manifest.mjs",
      version: 1,
    },
    verifier: {
      identity: "scripts/verify-stage-18e-h-subject-manifest.mjs",
      command: "npm run verify:stage-18e-h-subject-manifest",
