...
async function runValidationPresentationRegressions(): Promise<void> {
  ...
  for (const candidate of finiteOutcomes) {
    ...
    try {
      ...
      const recordedCheckedAt = completedRefreshFailure.control.metadata.snapshot().slots.anthropic?.lastValidationAttempt?.checkedAt;
      if (recordedCheckedAt === undefined) throw new Error("SMOKE_VALIDATION_CHECKED_AT_MISSING");
      const presentation = await page<Record<string, unknown>>(completedRefreshFailure.window, `(() => { const primary = document.querySelector('[data-notice-kind="outcome"]')?.textContent ?? ""; const warning = document.querySelector('[data-notice-kind="refresh-warning"]')?.textContent ?? ""; const actions = [...document.querySelectorAll(".detail-card .button-row button")]; const formattedCheckedAt = new Intl.DateTimeFormat(document.documentElement.lang, { dateStyle: "medium", timeStyle: "short" }).format(new Date(${JSON.stringify(recordedCheckedAt)})); return { primary, warning, formattedCheckedAt, documentLanguage: document.documentElement.lang, allLocked: actions.length > 0 && actions.every((node) => node.hasAttribute("disabled")), reasons: actions.map((node) => node.getAttribute("title")) }; })()`);
      preservedOutcomes.push({ ...candidate, recordedCheckedAt, dispatches: completedRefreshFailure.validation.dispatches(), ...presentation });
    } finally { await closeSurface(completedRefreshFailure); }
  }
  record("finite-validation-outcomes-survive-refresh-failure", preservedOutcomes.length === 6 && preservedOutcomes.every((item) => item["dispatches"] === 1 && String(item["primary"]).includes(String(item["title"])) && String(item["primary"]).includes(String(item["fact"])) && item["documentLanguage"] === "en-GB" && String(item["primary"]).includes(`at ${String(item["formattedCheckedAt"])}`) && !/(?:just now|\d+ (?:minute|hour|day)s? ago)/u.test(String(item["primary"])) && String(item["warning"]).includes("Current details could not be refreshed") && item["allLocked"] === true && (item["reasons"] as unknown[]).every((reason) => String(reason).toLowerCase().includes("reopen"))), preservedOutcomes);
...
async function runMutationPresentationRegressions(): Promise<void> {
  ...
  try {
    ...
    const warningStateSource = `(() => { const actions = [...document.querySelectorAll(".detail-card .button-row button")]; const validate = actions.find((node) => node.textContent?.trim() === "Validate connection"); const storage = actions.filter((node) => node !== validate); return { tone: document.querySelector(".notice")?.getAttribute("data-tone"), notice: document.querySelector(".notice")?.textContent ?? "", validateEnabled: validate !== undefined && !validate.hasAttribute("disabled"), storageCount: storage.length, storageLocked: storage.every((node) => node.hasAttribute("disabled") && (node.getAttribute("title") ?? "").includes("Reopen credential setup")) }; })()`;
    await waitFor(warningAndCommittedValidation.window, `(() => { const state = ${warningStateSource}; return document.querySelector("dialog") === null && state.tone === "warn" && state.notice.includes("Saved securely") && state.notice.includes("could not be cleared") && state.validateEnabled === true && state.storageCount === 3 && state.storageLocked === true; })()`);
    const state = await page<Record<string, unknown>>(warningAndCommittedValidation.window, warningStateSource);
