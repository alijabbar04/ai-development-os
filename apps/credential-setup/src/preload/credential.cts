...
contextBridge.exposeInMainWorld("credentialVault", Object.freeze({
  ...
  rotate: (slotId: string, secret: string, credentialId: string, recordRevision: number, recordToken: string, clearClipboard: boolean, entryMode: string, nickname: string | null, ownership: string | null, authorizedBy: string | null) => ipcRenderer.invoke(CHANNELS.rotate, Object.freeze({ ...envelope(), slotId, secret, credentialId, recordRevision, recordToken, clearClipboard, entryMode, nickname, ownership, authorizedBy })),
