...
interface BootstrapOptions {
  ...
  exitElectron(app: BootstrapApp, code: number): void;
  forceExit(code: number): void;
}

interface BootstrapModule {
  boundedFailureLine(phase: unknown, code: unknown): string;
  startProductionCredentialBootstrap(options: BootstrapOptions): boolean;
}
...
function fixture(overrides: Partial<BootstrapOptions> = {}) {
  ...
  const options: BootstrapOptions = {
    ...
    loadMain() { events.push("load-main"); return Promise.resolve(); },
    writeLine(line) { lines.push(line); },
    setExitCode(code) { events.push(`process-exit-code:${code}`); },
    exitElectron(target, code) { events.push(`electron-exit:${code}`); target.exit(code); },
    forceExit(code) { events.push(`force-exit:${code}`); },
    ...overrides,
  };
  return { app, events, lines, options };
}
...
describe("synchronous production startup bootstrap", () => {
  it("invokes the package-root entry exactly once without a require.main gate", () => {
    ...
    runInNewContext(entry, {
      ...
      require(specifier: string) {
        if (specifier !== "./startup-bootstrap-runtime.cjs") throw new Error("Unexpected bootstrap dependency.");
        return { boundedFailureLine: bootstrap.boundedFailureLine, startProductionCredentialBootstrap };
      },
    });
  ...
  it("binds the exact runtime and registers the fixed privileged scheme before loading ESM", async () => {
    const f = fixture();
    expect(bootstrap.startProductionCredentialBootstrap(f.options)).toBe(true);
  ...
  it("refuses missing, malformed, and unreviewed runtime bindings with one finite line", () => {
    ...
    for (const [override, code] of cases) {
      ...
      expect(bootstrap.startProductionCredentialBootstrap(f.options)).toBe(false);
  ...
  it("contains hostile Electron binding proxies and accessors inside the finite boundary", () => {
    ...
    for (const binding of hostileBindings) {
      ...
      if (f.events.some((event) => event === "electron-exit:1")) expect(f.events).not.toContain("force-exit:1");
      else expect(f.events).toContain("force-exit:1");
    }
  });

  it("forces a finite pre-service exit only when no safe Electron exit remains", () => {
    const normal = fixture({ electronVersion: () => "43.4.0" });
    expect(bootstrap.startProductionCredentialBootstrap(normal.options)).toBe(false);
    expect(normal.events).toContain("electron-exit:1");
    ...
    const fallback = fixture({
      ...
      writeLine() { throw new Error("private-writer-canary"); },
      exitElectron() { throw new Error("private-exit-canary"); },
    });
    expect(bootstrap.startProductionCredentialBootstrap(fallback.options)).toBe(false);
    expect(fallback.lines).toEqual([]);
    expect(fallback.events).toContain("process-exit-code:1");
    expect(fallback.events).toContain("force-exit:1");
  });
  ...
  it("collapses protocol and ESM loader failures without copying hostile content", async () => {
    ...
    expect(bootstrap.startProductionCredentialBootstrap(protocol.options)).toBe(false);
    ...
    expect(bootstrap.startProductionCredentialBootstrap(loader.options)).toBe(true);
  ...
  it("clamps unreviewed failure fields to the finite bootstrap vocabulary", () => {
    const line = bootstrap.boundedFailureLine("private-phase\ncanary", "private-code\ncanary");
    expect(line.match(/\n/gu)).toHaveLength(1);
