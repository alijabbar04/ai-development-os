...
describe("flat hostile IPC projection", () => {
  it("parses every bounded request shape and preserves the secret for the foundation's one canonical trim", () => {
    ...
    expect(parseRotatePayload({ ...base, ...identity, secret: "SYNTHETIC_ROTATION_VALUE", clearClipboard: false }).operation).toBe("rotate");
  ...
  it("requires consistent ownership metadata and bounded authoritative identity tokens", () => {
    ...
    expect(() => parseRotatePayload({ ...base, ...identity, recordRevision: 0, secret: "SYNTHETIC", clearClipboard: true })).toThrow();
    expect(() => parseRotatePayload({ ...base, ...identity, recordToken: "short", secret: "SYNTHETIC", clearClipboard: true })).toThrow();
  });
});
